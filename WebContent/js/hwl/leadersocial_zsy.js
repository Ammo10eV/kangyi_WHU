...
require([ 'echarts', 'echarts/chart/force', // 使用柱状图就加载force模块，按需加载
	...
	var myChart = ec.init(document.getElementById('leadersocial_zsy'));
	function getNode(number) {
		var obj = new Array;
		$.ajax({
			url : "leaderServletzsy?method=getNode"+ number,
			type : "POST",
			dataType : "text",
			cache : false,
			async : false,
	...
	function getLink(number) {
		var obj = new Array;
		$.ajax({
			url : "leaderServletzsy?method=getLink"+ number,
			type : "POST",
			dataType : "text",
			cache : false,
			async : false,
	...
	function readNodes(number) {
		var obj = new Array;
		$.ajax({
			url : "leaderServletzsy?method=readNodes" + number,
			type : "POST",
			dataType : "text", 
			cache : false,
			async : false,
...
//			},
	...
	function readLinks(number) {
		var obj = new Array;
		$.ajax({
			url : "leaderServletzsy?method=readLinks" + number,
			type : "POST",
			dataType : "text",
			cache : false,
			async : false,
...
//			},
	...
	var option = {
			timeline : {
				data : [ '2011-03-29','2011-03-31','2011-04-02','2011-04-04',
				         '2011-04-06','2011-04-08','2011-04-10','2011-04-12'
				         ,'2011-04-14'
				         ], 
							label : {
