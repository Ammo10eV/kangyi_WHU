...
require([ 'echarts', 'echarts/chart/force', // 使用柱状图就加载force模块，按需加载
	...
	var myChart = ec.init(document.getElementById('leadersocial_zhoubin'));
	function getNode(number) {
		var obj = new Array;
		$.ajax({
			url : "leaderServletzhoubin?method=getNode"+ number,
			type : "POST",
			dataType : "text",
			cache : false,
			async : false,
	...
	function getLink(number) {
		var obj = new Array;
		$.ajax({
			url : "leaderServletzhoubin?method=getLink"+ number,
			type : "POST",
			dataType : "text",
			cache : false,
			async : false,
	...
	function readNodes(number) {
		var obj = new Array;
		$.ajax({
			url : "leaderServletzhoubin?method=readNodes" + number,
			type : "POST",
			dataType : "text", 
			cache : false,
			async : false,
...
//			},
	...
	function readLinks(number) {
		var obj = new Array;
		$.ajax({
			url : "leaderServletzhoubin?method=readLinks" + number,
			type : "POST",
			dataType : "text",
			cache : false,
			async : false,
